### Australian Shark-Bite Database code ###
...
#import data
setwd("~/.../")
ASID <- read.csv("Australian Shark-Incident Database r version.csv", fileEncoding = "Latin1", check.names = F, header=T, na.strings=c("", " ", "NA"))
...
#plot proportions
Injury.proportion <- ggplot(injuryprop, aes(x = Recovery.status, y = percentage, fill = injury.location)) +
  labs(fill='injury location') + 
  geom_bar(width = 0.9, stat = "identity") +
  scale_fill_manual(values = c("dodgerblue", "deepskyblue", "lightskyblue", "lightskyblue3")) +
  geom_text(aes(label = percentage), position = position_stack(vjust = 0.5), colour = "Black") +
  scale_y_discrete(expand = c(0,0)) +
  xlab("recovery status") +
  ylab("proportion of bites") +
  theme_classic()
Injury.proportion
